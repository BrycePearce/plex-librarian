...
Deno.test(
  ...
  async () => {
    ...
    const quickCleanupKey = queryKeys.staleQuickCleanup.analysis("2", 365);
    const previewKey = queryKeys.downloadCleanupPreview.forItems("2", ["42"]);

    queryClient.setQueryData(staleKey, { items: [] });
    queryClient.setQueryData(quickCleanupKey, { candidates: [] });
    ...
    assertEquals(queryClient.getQueryState(quickCleanupKey)?.isInvalidated, true);
...
Deno.test("server switch resets every server-scoped detail cache", async () => {
  ...
  const quickCleanupKey = queryKeys.staleQuickCleanup.analysis("1", 365);
