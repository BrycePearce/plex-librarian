...
export function ArrIntegrationDialog() {
  ...
  const { data: plexPathMappings } = useQuery({
    queryKey: queryKeys.settings.plexPathMappings,
  ...
  const [namespaceLibraryKey, setNamespaceLibraryKey] = useState("");
  const [plexPath, setPlexPath] = useState("");
  const [localPath, setLocalPath] = useState("");
  const [sampleRatingKey, setSampleRatingKey] = useState("");
  const [sampleMediaId, setSampleMediaId] = useState("");
  ...
  const createNamespaceMapping = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: queryKeys.settings.plexPathMappings });
      setPlexPath("");
      setLocalPath("");
      setSampleRatingKey("");
      setSampleMediaId("");
  ...
  const removeNamespaceMapping = useMutation({
    mutationFn: api.settings.deletePlexPathMapping,
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.settings.plexPathMappings }),
  });
