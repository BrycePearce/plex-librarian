...
Deno.test("audio starts with the original track and crossfades to the Act 1 boss", async () => {
...
Deno.test("a resumed later act still opens with the original main-branch song", async () => {
  const first = fakeAudio();
  const second = fakeAudio();
  const audio = new ArcadeAudio(
    [first.element, second.element],
...
Deno.test("an opening track started by navigation is adopted without restarting", () => {
  const first = fakeAudio();
  const second = fakeAudio();
...
Deno.test("music settings apply to both crossfade elements", () => {
  ...
  const audio = new ArcadeAudio([first.element, second.element], TRACKS, settings);
