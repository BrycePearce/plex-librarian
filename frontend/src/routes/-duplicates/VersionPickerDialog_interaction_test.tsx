...
for (const mediaType of ["episode", "movie"] as const) {
  Deno.test(`${mediaType} duplicate QB selection survives refreshed Arr and cleanup verification failure`, async () => {
    ...
    const item: DuplicateGroup = mediaType === "movie"
      ? {
        mediaType,
        ratingKey: "episode",
        libraryKey: "tv",
    ...
    const preview: VersionDeletionPreviewResponse = {
      mediaType,
      arrService: mediaType === "movie" ? "radarr" : "sonarr",
      availableVersions: [],
      versions: [{
        mediaId: 1,
        plexPaths: ["/tv/A.mkv"],
    ...
    const key = queryKeys.versionDeletionPreview.forVersions(mediaType, "episode", [1], true);
    ...
    try {
      ...
      const arr = () =>
        renderer!.root.findByType(DestinationOptions).props.options
          .find((option: { id: string }) => option.id === "arr");
      // The legacy individual-episode API owns Sonarr coordination; it has no opt-out.
      assertEquals(arr().disabled, mediaType === "episode");
      assertEquals(arr().checked, mediaType === "episode");
      if (mediaType === "movie") await act(() => arr().onChange(true));
      assertEquals(arr().checked, true);
      ...
      await act(async () => {
        client.setQueryData(key, {
          ...
          cleanupReason:
            "A retained Plex path has no single validated local mapping: /archive/B.mkv",
